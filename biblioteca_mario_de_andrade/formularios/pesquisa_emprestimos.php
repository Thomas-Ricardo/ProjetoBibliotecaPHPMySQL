...
include __DIR__ . ("/../classe/conexao.php");
    ...
    $sql = "SELECT * FROM `emprestimos` WHERE `usuario_emp` LIKE '%$pesquisa%'";

    $dados = mysqli_query($conn, $sql);

    ?>
    <h1><b>Pesquisar Empréstimos</b></h1>
...
ead>
        <tr>
            <td scope="col">Livro Emprestado</td>
            <td scope="col">Usuário</td>
            <td scope="col">Data do Empréstimo</td>
        ...
        while($linha = mysqli_fetch_assoc($dados)){
            ...
            $usuario_emp= $linha['usuario_emp'];
            $livro_emprestado = $linha['livro_emprestado'];           
            $data_emprestimo = $linha['data_emprestimo'];
            $quantidade_emp = $linha['quantidade_emp'];
           echo "<tr>
                    <td scope='row'>$livro_emprestado</td>
                    <td>$usuario_emp</td>
                    <td>$data_emprestimo</td>
                    <td>$quantidade_emp</td>    
                    <td width=150px><a href='http://localhost/biblioteca_mario_de_andrade/formularios/emprestimo_edit.php?id=$id_emprestimo' class='btn btn-success'>Editar</a>
                        <a href='#' class='btn btn-danger btn-sm' data-bs-toggle='modal' data-bs-target='#confirma' onclick=" .'"' . "get_data($id_emprestimo, 'O empréstimo de $usuario_emp, do livro $livro_emprestado, código: $id_emprestimo')" .'"'.">Deletar</a>
                    </td>    
                </tr>";
       } ?>
        </tbody>
    ...
    <div class="modal fade" id="confirma" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
        <div class="modal-dialog">
            ...
            <form action='http://localhost/biblioteca_mario_de_andrade/scripts/deletar_emprestimo_script.php' method='POST'>
                <p>Deseja realmente deletar</p>
                <p id='usuario_emprestimo'>Nome Usuário</p>
                </div>
...
<script type='text/javascript'>
    function get_data(id, nome) {
        ...
        document.getElementById('usuario_emprestimo').innerHTML = nome; 
    }
</script>
